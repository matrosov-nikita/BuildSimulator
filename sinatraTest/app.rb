...
get '/addBuilding' do
  ...
  if  valid_coordinates(x.to_i,y.to_i) && valid_string(type)
     return (Building.addBuidling(x,y,type)!=nil).to_s
  end
  "false"
end
...
get '/removeBuilding' do
  x = params['x']
  y = params['y']
   return Building.remove(x,y).to_s if valid_numbers(x,y)
  Building.generateXMLByBuilding x,y
end

get '/generateXMLByBuilding' do
  return Building.generateXMLByBuilding(params['x'].to_i,params['y'].to_i)
end
...
get '/moveBuilding' do
  ...
  new_x = params['new_x']
  new_y = params['new_y']
  if valid_numbers(x,y) && valid_coordinates(new_x.to_i,new_y.to_i)
    return Building.move(x,y,new_x,new_y).to_s
  end
  Building.generateXMLByBuilding(x,y)
end
...
get '/startContract' do
  ...
  contract = params['contract']
   if valid_numbers(x,y) && (config.keys[1]..config.keys[2]).include?(contract.to_i)
     return Building.startContract(x,y,contract).to_s
   end
  Building.generateXMLByBuilding x,y
end
...
get '/isBuildComplete' do
  x = params['x']
  y = params['y']
  return Building.isBuildComplete(x,y).to_s  if valid_numbers(x,y)
end

get '/getShopIncome' do
  x = params['x']
  y = params['y']
  return Building.getShopIncome(x,y).to_s if valid_numbers(x,y)
  Building.generateXMLByBuilding x,y
end
...
get '/getFactoryIncome' do
  x = params['x']
  y = params['y']
   return Building.getFactoryIncome(x,y).to_s if valid_numbers(x,y)
  Building.generateXMLByBuilding x,y
end
